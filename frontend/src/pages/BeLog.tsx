...
export default function BeLog() {
  ...
  useEffect(() => {
    ...
    const url = baseURL.replace(/\/api\/v1\/?$/, '') + '/api/v1/logs/stream?source=be';
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-900 overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-2 border-b border-slate-700 bg-slate-800">
        ...
        <h1 className="text-lg font-semibold text-slate-100">Backend (Uvicorn) Log</h1>
      ...
      <pre ref={preRef} className="flex-1 p-4 overflow-auto text-sm text-amber-200 font-mono whitespace-pre-wrap">
