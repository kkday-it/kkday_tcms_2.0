...
import { ClipboardList, Plus, Loader2, Trash2, Pencil, Folder as FolderIcon, Search } from 'lucide-react';
...
function EditPlanModal({ plan, folders, runs, cases, onClose, onSaved }: {
    plan: TestPlan | null;
    folders: PlanFolder[];
    runs: TestRun[];
    cases: TestCase[];
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
                ...
                <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between shrink-0">
                    <h2 className="text-lg font-semibold text-slate-800">{isNew ? 'New Test Plan' : 'Edit Test Plan'}</h2>
                    <button onClick={onClose} className="text-slate-400 hover:text-slate-600 p-1 hover:bg-slate-100 rounded-md">✕</button>
                </div>
                ...
                <form onSubmit={handleSave} className="flex flex-col flex-1 overflow-hidden">
                        ...
                        <div>
                            <div className="flex gap-1 border-b border-slate-200 mb-3">
                                {(['runs', 'cases'] as const).map(tab => (
                                    <button key={tab} type="button" onClick={() => setActiveTab(tab)}
                                        className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors ${activeTab === tab ? 'bg-white border border-b-white border-slate-200 text-primary-700 -mb-px' : 'text-slate-500 hover:text-slate-700'}`}>
                            ...
                            {activeTab === 'runs' && (
                                <div className="max-h-48 overflow-y-auto rounded-lg border border-slate-200 divide-y divide-slate-100">
                                    {runs.length === 0 && <p className="text-sm text-slate-400 p-3">No test runs available.</p>}
                                    {runs.map(run => (
                                        <label key={run.id} className="flex items-center gap-3 px-3 py-2 hover:bg-slate-50 cursor-pointer">
                                            ...
                                            <span className="text-sm text-slate-700">{run.title}</span>
                                        </label>
                                    ))}
                                </div>
                            )}
                                ...
                                <>
                                    <div className="relative mb-2">
                                        ...
                                        <input type="text" placeholder="Search cases…" value={caseSearch}
                                            ...
                                            className="w-full pl-9 pr-3 py-1.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" />
                                    </div>
                                    <div className="max-h-44 overflow-y-auto rounded-lg border border-slate-200 divide-y divide-slate-100">
                                        {filteredCases.length === 0 && <p className="text-sm text-slate-400 p-3">No cases found.</p>}
                                        {filteredCases.map(tc => (
                                            <label key={tc.id} className="flex items-center gap-3 px-3 py-2 hover:bg-slate-50 cursor-pointer">
                    ...
                    <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-end gap-3 shrink-0">
                        <button type="button" onClick={onClose} disabled={isSaving}
                            className="px-4 py-2 border border-slate-300 bg-white text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 disabled:opacity-50">
                            Cancel
                        </button>
                        <button type="submit" disabled={isSaving || !title.trim()} className="btn-primary flex items-center gap-2 min-w-[100px] justify-center">
